...
- Full backend scopes after Director additions: **612 passed, 3 skipped**. Skips document Windows FFmpeg/NumPy analysis instead of Librosa JIT. A first rerun failed fixture setup because an old pytest temp directory was inaccessible; a fresh explicit `--basetemp` resolved that environmental error.
...
Remaining immediate internal-engine work: serialize competing model loads, add the resolved Automatic/Fast/Quality/Maximum readiness surface, and verify cancellation and inference with actual installed weights. The pinned model has not yet been downloaded or qualified. The new worker memory admission check is conservative and is not hardware qualification evidence. The current running backend has not been restarted for this catalog change yet.
